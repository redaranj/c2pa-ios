  ...
  private func webView(_ webView: WKWebView, runOpenPanelWith parameters: WKOpenPanelParameters, initiatedByFrame frame: WKFrameInfo, completionHandler: @escaping ([URL]?) -> Void) {
    ...
    private func showSignedPhotoPicker() {
        ...
        guard let rootVC = self.window?.rootViewController,
              let topVC = findTopViewController(rootVC) else {
        ...
        if photos.isEmpty {
            ...
            let alert = UIAlertController(title: "No Signed Photos", message: "Take a photo with C2PA Example first to add credentials.", preferredStyle: .alert)
      ...
      private func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
            if let url = navigationAction.request.url {
                ...
                if url.host == "contentcredentials.org" || 
                   url.host == "www.contentcredentials.org" ||
                   url.host == "verify.contentauthenticity.org" ||
                   url.scheme == "file" {
...
struct VerifyWebView: View {
    ...
    var body: some View {
        NavigationView {
            ...
            WebView(url: URL(string: "https://contentcredentials.org/verify")!, shouldReload: $shouldReloadWebView)
